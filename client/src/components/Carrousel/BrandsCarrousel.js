...
class BrandsCarrousel extends Component {
    ...
    render() {
        ...
        const listItems = imgs.slice(0, 10).map((img) => {
            return (
                <div key={img.id.toString()}>
                    {/* <Link to={ `/productos/marcas/${ img.slug }` }> */}<img id="brandCarrouselImg" src={img.img} alt={img.alt} />{/* </Link> */}
                </div>
            );
        });
        const _listItems = imgs.slice(0, 10).map((img) => {
            return (
